...
from dynamic_programming import action_grid, solve_reference
...
def make_features(case, s_now, use_look=True):
    h = case["hour"]
    cols = [
...
def rollout_raw_dd(model, x_mean, x_std, y_mean, y_std, case, s0, start, stop):
    ...
    s_now = s0
    base = make_features(case, np.zeros(case["p_l"].size), use_look=True)
...
def greedy_rollout(case, s0, start, stop):
    actions = action_grid()
    n = stop - start
    p_g = np.zeros(n)
    p_c = np.zeros(n)
    ...
    for i, t in enumerate(range(start, stop)):
        best_cost = np.inf
        best = (0.0, 0.0, s_now, 0.0, 0.0)
        for act in actions:
            pc, pd, s_next, pg = hard_layer(s_now, float(act), case["p_l"][t], case["p_pv"][t])
            cost = float(step_cost(pg, pc, pd, case["c_b"][t], case["c_s"][t]))
            if cost < best_cost:
                best_cost = cost
                best = (float(pc), float(pd), float(s_next), float(pg), float(pc - pd))
        p_c[i], p_d[i], s_next, p_g[i], u[i] = best
        s[i] = s_now
        s_now = s_next
    s_next_path = np.concatenate([s[1:], np.array([s_now])])
    return p_g, p_c, p_d, u, s, s_next_path
...
def plot_figures(case, ref, pe, dd_rep, dd_raw, start, n_test):
    ...
    axes[0].plot(hours, case["p_l"][start : start + 24], color="#2c5f8a", label=r"$P_{\mathrm{L}}$")
    axes[0].plot(hours, case["p_pv"][start : start + 24], color="#d08a2b", label=r"$P_{\mathrm{PV}}$")
    axes[0].set_ylabel(r"$P$ (kW)")
    axes[0].legend(loc="upper right", frameon=False)
    axes[1].axhline(0.90, color="0.75", ls=":", lw=0.8)
    axes[1].axhline(0.20, color="0.75", ls=":", lw=0.8)
    axes[1].plot(hours, ref["s"][start : start + 24], color="#2aa198", label="DP")
    axes[1].plot(hours, dd_rep["s"][sl], color="#d62728", ls="--", label="DD")
    axes[1].plot(hours, pe["s"][sl], color="#4b3f91", label="PE")
    ...
    ax.plot(np.arange(week), r_dd, color="#d62728", lw=1.1, label="DD")
    ax.plot(np.arange(week), r_pe, color="#4b3f91", lw=1.4, label="PE")
    ax.set_xlabel(r"$t$ (h)")
    ax.set_ylabel(r"$|r_{\mathrm{G}}|$ (kW)")
    ...
    costs = [
        float(
            np.sum(
                step_cost(
                    ref["p_g"][start : start + n_test],
                    ref["p_c"][start : start + n_test],
                    ref["p_d"][start : start + n_test],
                    case["c_b"][start : start + n_test],
                    case["c_s"][start : start + n_test],
                )
            )
        )
        / n_days,
        float(
            np.sum(
                step_cost(
                    case["p_l"][start : start + n_test] - case["p_pv"][start : start + n_test],
                    np.zeros(n_test),
                    np.zeros(n_test),
                    case["c_b"][start : start + n_test],
                    case["c_s"][start : start + n_test],
...
def main():
    ...
    x_nl = make_features(case, ref["s"], use_look=False)
    ...
    nl_mean, nl_std = standardize_fit(x_nl[sl_tr])
    x_tr_nl = apply_standard(x_nl[sl_tr], nl_mean, nl_std)
    x_va_nl = apply_standard(x_nl[sl_va], nl_mean, nl_std)

    dd_acc, pe_acc, dd_feas, pe_feas, dd_cost, pe_cost = [], [], [], [], [], []
    pe_nl_acc, pe_nl_cost = [], []
    last_pe = last_dd_rep = last_dd_raw = None
    feat_look = make_features(case, np.zeros(n), use_look=True)
    ...
    for seed in SEEDS:
        ...
        def pe_nl_cmd(t, s_now, model=pe_nl_net):
            feat = feat_nl[t].copy()
            feat[7] = s_now
            z = model.predict(apply_standard(feat[None, :], nl_mean, nl_std))[0, 0]
            return P_MAX * np.tanh(z)
        ...
        pg_n, pc_n, pd_n, u_n, s_n, sn_n = rollout_hard(pe_nl_cmd, case, ref["s"][start], start, n)
        ...
        dd_cost.append(float(np.sum(step_cost(pg_r, pc_r, pd_r, case["c_b"][sl_te], case["c_s"][sl_te]))))
        pe_cost.append(float(np.sum(step_cost(pg_p, pc_p, pd_p, case["c_b"][sl_te], case["c_s"][sl_te]))))
        pe_nl_cost.append(float(np.sum(step_cost(pg_n, pc_n, pd_n, case["c_b"][sl_te], case["c_s"][sl_te]))))
    ...
    gr_cost = float(np.sum(step_cost(gr_pg, gr_pc, gr_pd, case["c_b"][sl_te], case["c_s"][sl_te])))
    ...
    metrics = {
        ...
        "pe_nl_cost_mean": mean_std(pe_nl_cost)[0],
        "pe_nl_cost_std": mean_std(pe_nl_cost)[1],
        ...
        "pe_nl_mae_g": mean_std([d["mae_g"] for d in pe_nl_acc]),
        "pe_nl_mae_u": mean_std([d["mae_u"] for d in pe_nl_acc]),
        "pe_nl_mae_s": mean_std([d["mae_s"] for d in pe_nl_acc]),
        ...
        "nb_mse": mse_comp(
            nb_pg, nb_pc, nb_pd, nb_s, ref["p_g"][sl_te], ref["p_c"][sl_te], ref["p_d"][sl_te], ref["s_next"][sl_te]
        ),
        "greedy_mae_g": mae(gr_pg, ref["p_g"][sl_te]),
        "greedy_mae_u": mae(gr_u, ref["u"][sl_te]),
        "greedy_mae_s": mae(gr_s, ref["s"][sl_te]),
    }
